<#
    ...
    .DESCRIPTION
        ...
        - It will replace the word 'Confluence' with 'Hudu' anywhere it is stated
        - It will highlight in red any links to other Confluence pages (as they will be broken links once docs are migrated to Hudu, and require manual fixing)
        
        Not supported / Known issues:
        - Image captions (Confluence does not export these with the HTML for some strange reason)
        - Rich text or macros inside callouts (i.e. Images, code, or headings inside info/warning callouts, as Hudu does not support this)
        - Probably a fair bit of other stuff too
...
foreach ($htmlFile in $htmlFiles) {
    ...
    $patternReplacements = @{
        # Replace References to the word 'Confluence'
        'Confluence'  = 'Hudu'
        ...
        # Get rid of footer
        '<div id="footer" role="contentinfo"><section class="footer-body"><p>Document generated by Confluence (.*?)</p><div id="footer-logo"><a href="http://www.atlassian.com/">Atlassian</a></div></section></div>' = ''
        # Add space in TOC
        '<li><span class=''TOCOutline''>(.*?)</span>' = '<li><span class=''TOCOutline''>$1. </span>'
        # Highlight links to other confluence pages (i.e. will become broken links)
        '<a href="https://(.*?).atlassian.net/wiki/spaces/(.*?)" (.*?) data-linked-resource-type="(.*?)">(.*?)</a>' = '<strong><a href="https://$1.atlassian.net/wiki/spaces/$2" style="color: red;">BROKEN LINK: $5 [https://$1.atlassian.net/wiki/spaces/$2]</a></strong>'
        # Remove Space name from heading
        '<h1 id="title-heading" class="pagetitle"><span id="title-text"> (.*?) : (.*?)</span></h1>' = '<h1 id="title-heading" class="pagetitle"><span id="title-text">$2</span></h1>'
    }
